...
use hickory_server::resolver::{config::*, lookup::Lookup, system_conf::parse_resolv_conf};
use anyhow::Result;
...
impl std::fmt::Debug for Client {
  fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
    f.debug_struct("Client").field("resolver", &self.config).finish()
  }
}
...
impl Client {
  pub fn from_conf(conf: &str) -> Result<Self> {
    ...
    let resolver = AsyncResolver::new(config.clone(), options.clone(), TokioConnectionProvider::default());
    Ok(Self { config: (config, options), resolver })
  }

  pub async fn resolve(&self, domain: &str, record_type: RecordType) -> Result<Lookup> {
